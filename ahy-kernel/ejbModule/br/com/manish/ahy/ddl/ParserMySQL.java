...
import br.com.manish.ahy.util.DAOUtil;
import br.com.manish.ahy.util.JPAUtil;
import br.com.manish.ahy.util.TextUtil;
...
public class ParserMySQL implements Parser {
	...
	private String createColumn(Element el, String table) {
		...
		if (el.getAttributeValue("maxLength") != null
				&& !el.getAttributeValue("maxLength").equals("")) {
		...
		if (el.getAttributeValue("default") != null
				&& !el.getAttributeValue("default").equals("")) {
			sql += " DEFAULT " + el.getAttributeValue("default");
		}
		sql += ", ";
		if (Boolean.valueOf(el.getAttributeValue("primaryKey"))) {
			sql += "PRIMARY KEY pk_" + TextUtil.tinyFirstLetter(table) + " ("
		...
		if (fk != null && !fk.equals("")) {
			...
			if (tokenFK.length != 2) {
				throw new OopsException(
						"Wrong foreign key instruction, need [Table.field] : ["
								+ fk + "]");
			}
			String nomeFK = "fk_" + TextUtil.tinyFirstLetter(table) + "_"
					+ TextUtil.tinyFirstLetter(tokenFK[1]);
			sql += "FOREIGN KEY " + nomeFK + " ("
					+ el.getAttributeValue("name") + ") REFERENCES "
					+ tokenFK[0] + "." + tokenFK[1] + "(" + tokenFK[2] + "), ";
		}
		if (Boolean.valueOf(el.getAttributeValue("unique"))) {
			sql += "UNIQUE uq_" + TextUtil.tinyFirstLetter(table)
					+ el.getAttributeValue("name") + " ("
					+ el.getAttributeValue("name") + "), ";
		}
		return sql;
	...
	private String formatValueForInsert(Element el) {
		...
		if (type.equals("java.lang.String")) {
			...
			ret = "STR_TO_DATE('" + value.substring(0, 19)
					+ "', '%Y-%m-%d %H:%i:%s')";
